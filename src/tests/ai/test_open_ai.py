...
async def test_normal_conversation() -> None:
    conversation: Conversation = Conversation.get_new_conversation(LargeLanguageModel.GPT35_TURBO, prompt_template="You are only allowed to answer sarcastically")
...
async def test_function_call() -> None:
    ...
    conversation: Conversation = Conversation.get_new_conversation(LargeLanguageModel.GPT35_TURBO, function_list=[chat_gpt_function])
